...
## [impact statement]
Restricting network access to services may make troubleshooting and remediation of issues more complex. For example, if remote access to `etcd` is not enabled, certain diagnosis and management commands may be not be available.
